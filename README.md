# 🚀 給付金ナビ （支援金・補助金 自動収集＆閲覧アプリ）

本プロジェクト「給付金ナビ」は、自治体の公式ホームページ等から「給付金・助成金・支援金・補助金」に関する情報を自動で収集し、**ユーザーのプロフィール（世帯構成、所得、就労状況など）の条件に合致する制度のみを的確に判定・表示する**Webアプリケーションです。

現在は開発期間（演習）の都合上、**「京都市」のデータに特化**して稼働させていますが、バックエンドの収集パイプラインは検索クエリの対象ドメインを変更するだけで、**全国のあらゆる自治体に横展開できる拡張性の高い設計**となっています。
